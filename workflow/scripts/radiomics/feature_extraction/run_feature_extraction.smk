...
path_table = pd.DataFrame([
    {
        ...
        "mr_path": str(dmpdirs.PROCDATA / COMBINED_DATA_NAME / "images" /f"mit_{config['DATASET_NAME']}" / row["filepath_MR"]),
        "seg_path": dmpdirs.PROCDATA / COMBINED_DATA_NAME / "images" / f"processed_{config['DATASET_NAME']}" / row["PatientName"] / Path(row["filepath_SEG"]).parent.name / "bin_GTV.nii.gz"
}
...
rule crop_mri_seg:
    ...
    shell:
        """
        pixi run python workflow/scripts/radiomics/feature_extraction/preprocessing.py crop_mri_seg \
        --mr {input.MR_input} \
        --seg {input.SEG_input} \
...
rule MR_bias_correct:
    ...
    shell:
        "pixi run python workflow/scripts/radiomics/feature_extraction/preprocessing.py MR_bias_correct "
...
rule extract_feature:
    ...
    shell:
        "pixi run python workflow/scripts/radiomics/feature_extraction/preprocessing.py extract_feature "
...
rule merge_features:
    input:
        feature_csvs=[dmpdirs.PROCDATA / f"{COMBINED_DATA_NAME}/features/tmp_{p}.csv" for p in patients]
    output:
        merged_features=dmpdirs.RESULTS / f"radiomics/{COMBINED_DATA_NAME}/feature_extracted.csv"
    shell:
        """
        pixi run python workflow/scripts/radiomics/feature_extraction/preprocessing.py merge_features \
        --inputs {input} \
        --output {output}
        """
